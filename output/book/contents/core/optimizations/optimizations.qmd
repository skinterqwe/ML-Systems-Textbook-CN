...
#### 性能提升 {#sec-model-optimizations-performance-gains-b199}

@fig-quantization_impact 展示了量化对推理时间和模型大小的影响，采用了带双轴表示的堆叠柱状图。每个类别中左侧的柱子显示从 FP32 转向 INT8 时推理时间的改善，而右侧的柱子则描绘了相应的模型大小缩减。结果表明，量化模型可实现高达$4\times$倍更快的推理，同时将存储需求降低$4\times$倍，因此非常适合部署在资源受限的环境中。
...
### 数值格式比较 {#sec-model-optimizations-numerical-format-comparison-e4ad}

@tbl-numerics 比较了机器学习中常用的数值精度格式，强调了它们在存储效率、计算速度和功耗方面的权衡。FP8 和 TF32 等新兴格式也被引入，以进一步优化性能，尤其是在人工智能加速器上。
...
### 硬件特定优化库 {#sec-model-optimizations-hardwarespecific-optimization-libraries-a193}

@sec-ai-frameworks 中所涵盖的现代机器学习框架里的硬件优化库，使优化后的模型能够在不同硬件平台上高效部署。这些库可直接与训练和部署流水线集成，在模型表示、数值精度和架构效率等多个维度上，为各类优化技术提供针对硬件的加速。
