...
+:=========================+:=========================+:===========================+:=============================+
| **内存占用**             | 模型权重 + 单个激活图     | 权重 + 完整激活缓存 +      | 增加 3-5$\times$倍；  |
|                          |                          | 梯度 + 优化器状态          | 迫使采用激进压缩             |
+--------------------------+--------------------------+----------------------------+------------------------------+
| **计算操作**             | 仅前向传播               | 前向传播 + 反向传播 +      | 增加 2-3$\times$倍；  |
|                          |                          | 权重更新                  | 限制模型复杂度               |
+--------------------------+--------------------------+----------------------------+------------------------------+
| **内存带宽**             | 顺序读取权重             | 用于梯度的双向数据流        | 增加 5-10$\times$倍； |
|                          |                          |                            | 造成瓶颈                     |
+--------------------------+--------------------------+----------------------------+------------------------------+
| **每样本能耗**           | 单次推理操作             | 带收敛过程的多次梯度步      | 增加 10-50$\times$倍；|
|                          |                          |                            | 需要机会性调度               |
+--------------------------+--------------------------+----------------------------+------------------------------+
| **数据需求**             | 预先收集、经过整理的      | 稀疏、噪声较多、流式的      | 需要样本高效方法             |
|                          | 数据集                   | 本地数据                   |                              |
+--------------------------+--------------------------+----------------------------+------------------------------+
| **硬件利用**             | 针对前向传播优化         | 反向传播所需的不同访问模式  | 推理加速器                  |
|                          |                          |                            | 可能不利于训练               |
+--------------------------+--------------------------+----------------------------+------------------------------+
...
+:========================+:======================+:============================+:===========================+
| **少样本适配**          | 少量带标签数据集       | 低                           | 个性化、快速               |
|                         |（K-shot）             |                             | 端侧微调                   |
+-------------------------+-----------------------+-----------------------------+----------------------------+
| **经验回放**            | 流式数据              | 中等（缓冲区与更新）         | 非平稳数据，               |
|                         |                       |                             | 抗漂移稳定性              |
+-------------------------+-----------------------+-----------------------------+----------------------------+
| **压缩表示**            | 无标签或编码后的数据  | 低到中等                     | 内存受限设备，             |
|                         |                       |                             | 隐私敏感场景              |
+-------------------------+-----------------------+-----------------------------+----------------------------+
