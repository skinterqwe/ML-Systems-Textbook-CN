...
这种惊人的能耗反映了 AI 在各行业的快速普及。如@fig-ai-data-center-demand 所示，AI 工作负载的能源需求预计将增加数据中心的总能源消耗，特别是在 2024 年之后。尽管效率的提升抵消了不断上升的电力需求，但这些提升的速度正在放缓，从而放大了 AI 的环境影响。

```{python}
#| label: fig-ai-data-center-demand
#| fig-cap: "**预计需求**：到 2030 年，AI 工作负载将显著增加数据中心的电力需求，超过此前的效率提升。这凸显了 AI 系统日益增长的环境影响。数据来源：[@masanet2020energy]、Cisco、IEA、Goldman Sachs 全球投资研究。"
#| fig-align: center
#| echo: false
#| message: false
#| warning: false
...
### 分布式系统能耗优化 {#sec-sustainable-ai-distributed-systems-energy-optimization-5e83}
...
#### AI 与其他行业的能耗对比 {#sec-sustainable-ai-ai-energy-consumption-compared-industries-3d5d}
...
\node[above=0 of LCA](LCA){AI System};
 \end{tikzpicture}
```
**AI 系统生命周期**：通过设计、制造、使用和处置四个阶段分析 AI 系统，可以揭示超越运行能耗的全生命周期环境影响，包括资源枯竭和电子垃圾。这种生命周期评估能够进行有针对性的干预，以改善 AI 系统整个存在期内的可持续性。
:::

每个生命周期阶段都呈现出独特的环境影响和可持续性挑战，从设计优化到制造再到部署运行。

### 设计阶段 {#sec-sustainable-ai-design-phase-0954}

AI 系统的设计阶段涵盖了部署前机器学习模型的研究、开发和优化。这一阶段涉及迭代模型架构、调整超参数以及运行训练实验以提高性能。这些过程计算密集，需要大量使用硬件资源和能源。AI 模型设计的环境成本往往被低估，但反复的训练运行、算法改进和探索性实验对 AI 系统的整体可持续性影响贡献显著。

开发一个 AI 模型需要运行多次实验以确定最有效的架构。例如，自动架构搜索技术通过评估数百甚至数千种配置[^fn-architecture-search]（每种都需要独立的训练周期）来自动化选择最佳模型结构的过程。类似地，超参数调优涉及修改学习率、批大小和优化策略等参数以增强模型性能，通常采用穷举搜索技术。预训练和微调进一步增加了计算需求，因为模型在部署前需要在不同的数据集上经历多次训练迭代。这一过程的迭代性导致了高能耗，其中超参数调优和架构搜索对训练相关排放的贡献尤为突出[@strubell2019energy]。

[^fn-architecture-search]: **架构搜索（Architecture Search）**：用于寻找最优模型结构的自动化方法。神经架构搜索（NAS）技术详见 @sec-model-optimizations-neural-architecture-search-3915。

设计阶段的能耗规模在考虑 GPT-3 等大型语言模型时变得显而易见。报告的训练能耗仅反映了最终的训练运行，未考虑模型选择前大量的试错过程，这意味着实际消耗可能要高得多。在深度强化学习应用中，如 DeepMind 的 AlphaZero，模型需要经历反复的训练周期来改进决策策略，进一步放大了能源需求。

AI 模型设计的碳足迹因所需计算资源和为训练数据中心供电的能源来源而显著不同。一项被广泛引用的研究发现，训练单个大规模 NLP 模型可能产生的排放相当于五辆汽车整个生命周期的碳足迹[@strubell2019energy]。当训练在依赖化石燃料的数据中心进行时，影响更为显著。例如，在弗吉尼亚州（美国）燃煤设施中训练的模型，其排放远高于在水电或核能地区训练的模型。硬件选择也会显著影响排放；在能效较高的张量处理单元（TPU）上训练，与使用传统图形处理单元（GPU）相比，可以显著减少排放。

@tbl-training-emissions 总结了训练各种 AI 模型的估计碳排放量，说明了模型复杂度与环境影响之间的相关性。

| **AI 模型** | **训练 FLOPs** | **估计 $\textrm{CO}_2$ 排放量 (kg)** | **等效汽车行驶里程** |
|:---|:---:|:---:|:---:|
...
: **模型碳足迹**：训练大型 AI 模型会产生大量碳排放，与以 FLOPs 衡量的计算需求直接相关；例如，训练 GPT-3 所需的能源相当于数百辆汽车整个生命周期的排放量。了解这些排放对于制定可持续的 AI 实践和选择能效较高的硬件（如 TPU）以最小化环境影响至关重要。来源：{#tbl-training-emissions}

应对设计阶段的可持续性挑战需要在训练效率和计算资源管理方面进行创新。研究人员已经探索了稀疏训练、低精度算术和权重共享等技术，在不牺牲模型性能的情况下减少所需的计算量。使用预训练模型作为减少资源消耗的手段也越来越受到关注。研究人员可以微调预训练网络的较小版本，利用现有知识以更低的计算成本达到类似的结果，而不是从头开始训练模型。

优化模型搜索算法进一步促进了可持续性。传统的神经架构搜索方法需要评估大量的候选架构，但最近能效感知 NAS 方法的进展通过减少识别最优配置所需的训练迭代次数来优先考虑效率。公司也开始实施碳感知计算策略，通过在电网碳强度较低的时段调度训练任务，或将工作负载转移到拥有更清洁能源的数据中心[@gupta2022]。

设计阶段为整个 AI 生命周期奠定了基础，影响着训练和推理阶段的能源需求。随着 AI 模型复杂度的增长，必须重新评估其开发过程，以确保可持续性考量被整合到每个阶段。模型设计阶段做出的决策不仅决定了计算效率，还塑造了 AI 技术的长期环境足迹。

### 制造阶段 {#sec-sustainable-ai-manufacturing-phase-4ce1}

AI 系统的制造阶段是其生命周期中资源密集度最高的环节之一，涉及 GPU、TPU、FPGA 和其他 AI 加速器等专用半导体硬件的制造。这些芯片的生产需要大规模工业流程，包括原材料提取、晶圆制造、光刻、掺杂和封装——所有这些都对环境影响有重大贡献[@nakano2021geopolitics]。这一阶段不仅涉及高能耗，还产生有害废物，依赖稀缺材料，并对资源枯竭有长期影响。

#### 制造材料 {#sec-sustainable-ai-fabrication-materials-e46d}

AI 硬件的基础在于半导体，主要是驱动 AI 加速器的硅基集成电路。然而，现代 AI 芯片不仅依赖硅；它们还需要镓、铟、砷和氦等特种材料，每种材料都有独特的环境开采成本。由于其稀缺性、地缘政治敏感性以及与采矿和精炼相关的高能源成本，这些材料通常被归类为重要元素[@nakano2021geopolitics]。

硅本身储量丰富，但将其精炼为高纯度晶圆需要大量的能源密集型工艺。生产一块 300 毫米硅晶圆需要超过 8,300 升水，以及用于蚀刻和清洗的强酸，如氢氟酸、硫酸和硝酸[@cope2009pure]。半导体制造对超纯水的需求给当地水资源供应带来了巨大负担，领先制造厂每天消耗数百万加仑的水。

除了硅之外，镓和铟对高性能复合半导体至关重要，例如用于高速 AI 加速器和 5G 通信的半导体。美国地质调查局已将铟列为严重濒危材料，按当前消费速度估计，全球供应量可能不足 15 年[@davies2011endangered]。与此同时，氦是芯片生产中必不可少的冷却剂，是一种不可再生资源，一旦释放就会逸出地球引力场，使其永久不可回收。AI 硬件制造的持续扩张正在加速这些关键元素的枯竭，引发了对长期可持续性的担忧。

EUV 光刻技术的使用进一步放大了半导体制造的环境负担，这是制造 5nm 以下芯片所需的工艺。EUV 系统消耗大量能源，需要高功率激光器和复杂的光学器件。国际半导体路线图估计，每台 EUV 设备消耗约一兆瓦（MW）的电力，显著增加了尖端芯片生产的碳足迹。

#### 制造能耗 {#sec-sustainable-ai-manufacturing-energy-consumption-dc21}

制造 AI 硬件所需的能源量是巨大的，每块芯片的总能源成本往往超过其整个运行生命周期的能源使用量。制造单个 AI 加速器可能产生的碳排放量超过其在数据中心连续使用数年的排放量，这使得制造成为 AI 环境影响中的一个关键热点。

#### 有害废物和用水量 {#sec-sustainable-ai-hazardous-waste-water-usage-fc0f}

半导体制造还产生大量有害废物，包括气体排放、挥发性有机化合物（VOCs）、化学废水和固体副产品。芯片生产中使用的酸和溶剂会产生有毒废物流，需要专门处理以防止对周围生态系统造成污染。尽管废水处理技术取得了进步，但微量的金属和化学残留物仍可能被排放到河流和湖泊中，影响水生生物多样性和人类健康[@prakash2022cfu]。

半导体制造厂的用水需求也引发了对地区水资源紧张的担忧。台积电在亚利桑那州的制造厂预计每天消耗 890 万加仑的水，这一数字约占该市供水量的近 3%。虽然一些制造厂已开始投资水循环利用系统，但这些努力仍不足以抵消不断增长的需求。

#### 可持续倡议 {#sec-sustainable-ai-sustainable-initiatives-145c}

认识到半导体制造的可持续性挑战，行业领导者已开始实施减少能耗、废物产生和排放的倡议。Intel、台积电和三星等公司已承诺通过几个关键途径向碳中和半导体制造转型。许多制造厂正在整合可再生能源，台湾和欧洲的设施越来越多地使用水电和风能供电。通过减少对当地供水依赖的闭环循环系统，节水努力得到了扩展。制造工艺正在被重新设计，采用环保的蚀刻和光刻技术以最小化有害废物的产生。公司正在开发节能芯片架构，如针对每瓦性能优化的低功耗 AI 加速器，以减少制造和运行的环境影响。尽管付出了这些努力，随着 AI 加速器需求的不断攀升，AI 芯片制造的整体环境足迹仍在持续增长。如果材料效率、回收利用和制造技术没有重大改进，制造阶段将继续是 AI 可持续性挑战的主要贡献者。优化硬件架构能效的互补性挑战在 @sec-ai-acceleration 中讨论，运营基础设施可持续性在 @sec-ml-operations 中介绍，减少计算需求的算法技术详见 @sec-model-optimizations。

AI 硬件的制造阶段代表了 AI 生命周期中资源最密集、环境影响最大的环节之一。关键材料的提取、高能耗的制造工艺和有害废物的产生都助长了 AI 不断增长的碳足迹。虽然业界朝着可持续半导体制造的努力正在加速，但要将这些倡议扩展到满足不断增长的 AI 需求仍是一项重大挑战。

解决 AI 硬件的可持续性问题需要材料创新、供应链透明度和对强调芯片回收与再利用的循环经济模式的更大投资相结合。随着 AI 系统的不断发展，其长期可行性不仅取决于计算效率，还取决于减少底层硬件基础设施的环境负担。

### 使用阶段 {#sec-sustainable-ai-use-phase-a0a1}

AI 系统的使用阶段是其生命周期中能源密集的环节，涵盖训练和推理工作负载。随着 AI 在各行业的采用不断增长，开发和部署模型的计算需求持续增加，导致更大的能源消耗和碳排放。AI 系统的运营成本不仅限于处理中直接使用的电力；还包括支持大规模 AI 工作负载的数据中心、冷却基础设施和网络设备的电力需求。了解这一阶段的可持续性挑战对于减轻 AI 的长期环境影响至关重要。

AI 模型训练是使用阶段中计算成本最高的活动之一。训练大规模模型需要在 GPU 和 TPU 等专用硬件上运行数十亿甚至数万亿次数学运算，持续时间很长。近年来，随着 AI 模型复杂度的增长，训练的能耗急剧上升。大型语言模型训练表明，训练运行的碳足迹在很大程度上取决于执行训练的数据中心的能源结构。在主要依赖化石燃料的地区（如弗吉尼亚的燃煤数据中心）训练的模型，其排放量远高于在水电或核能设施中训练的模型。

除了训练之外，AI 的能源需求并不随着模型开发完成而结束。推理阶段产生的持续计算成本会随着部署规模和使用频率的增加而扩大。在实际应用中，推理工作负载持续运行，每天通过搜索引擎、推荐系统、语言模型和自主系统等服务处理数十亿次请求。虽然训练运行是能源密集型的，但为数百万用户提供服务的推理工作负载随着时间推移可能消耗更多的电力。研究表明，推理现在占 AI 相关总能耗的 60% 以上，在许多情况下超过了训练的碳足迹[@patterson2022carbon]。

数据中心在赋能 AI 方面发挥着核心作用，容纳了训练和推理所需的计算基础设施。这些设施依赖数千台高性能服务器，每台都消耗大量电力来处理 AI 工作负载。数据中心的电源使用效率（PUE）衡量其能源使用效率，直接影响 AI 的碳足迹。许多现代数据中心的 PUE 值在 1.1 到 1.5 之间，这意味着每用于计算的一个单位电力，另有 10% 到 50% 的电力消耗在冷却、电力转换和基础设施开销上[@barroso2019datacenter]。冷却系统是数据中心能耗的主要贡献者，因为 AI 加速器在运行过程中会产生大量热量。

数据中心的地理位置对其可持续性有直接影响。位于可再生能源可用地区的设施，与依赖化石燃料电网的设施相比，可以显著减少排放。Google 和 Microsoft 等公司已投资碳感知计算策略，在可再生能源发电量高的时段调度 AI 工作负载，以最小化其碳影响[@gupta2022]。

AI 日益增长的能源需求引发了对电网容量和可持续性权衡的担忧。AI 工作负载往往与其他高能耗行业（如制造业和交通运输）争夺有限的电力供应。在一些地区，AI 驱动的数据中心的增长导致电网压力增大，需要新的基础设施投资。所谓的"鸭子曲线"问题——可再生能源发电量在一天中波动——为平衡 AI 的能源需求与电网可用性带来了额外挑战。向分布式 AI 计算和边缘处理的转变正成为减少对集中式数据中心依赖的潜在解决方案，将部分计算任务转移到更靠近终端用户的位置。

减轻 AI 使用阶段的环境影响需要硬件、软件和基础设施层面的优化相结合。节能芯片架构（如低功耗 AI 加速器和专用推理硬件）的进步在减少每次查询能耗方面显示出前景。AI 模型本身正在通过量化、剪枝和蒸馏等技术进行效率优化，这些技术允许更小、更快的模型在保持高精度的同时需要更少的计算资源。与此同时，冷却效率、可再生能源整合和数据中心运营的持续改进对于确保 AI 不断增长的足迹长期可持续至关重要。

随着 AI 采用的持续扩展，能效必须成为模型部署策略中的核心考量。使用阶段将继续是 AI 环境足迹的主要贡献者，如果没有重大干预，该行业的电力消耗可能会呈指数级增长。可持续的 AI 开发需要行业、学术界和政策制定者之间的协调努力，以促进负责任的 AI 部署，同时确保技术进步不以长期环境可持续性为代价。

### 处置阶段 {#sec-sustainable-ai-disposal-phase-b4f1}

AI 系统的处置阶段在可持续性讨论中经常被忽视，但它带来了重大的环境挑战。AI 硬件的快速发展导致硬件寿命缩短，加剧了电子垃圾（e-waste）的增长和资源枯竭。随着 AI 加速器、GPU 和高性能处理器在几年内变得过时，管理其处置已成为一个紧迫的可持续性问题。与传统计算设备不同，AI 硬件包含复杂材料、稀土元素和有害物质，使回收和废物管理工作变得复杂。如果没有有效的策略来重新利用、回收或安全处置 AI 硬件，AI 基础设施的环境负担将继续升级。

AI 硬件的寿命相对较短，特别是在数据中心中，性能效率决定了频繁升级。平均而言，GPU、TPU 和 AI 加速器每三到五年更换一次，因为更新、更强大的型号不断进入市场。这种快速更替导致了持续的硬件处置循环，大规模 AI 部署产生了大量电子垃圾。与可能有转售或再利用二级市场的消费电子产品不同，AI 加速器一旦不再是最新技术，往往就不适合商业用途。对更快、更高效 AI 模型的追求加速了这一循环，导致越来越多的高性能计算硬件被丢弃。

AI 硬件处置的主要环境问题之一是有害物质的存在。AI 加速器含有铅、镉和汞等重金属，以及半导体制造中使用的有毒化合物。如果不妥善处理，这些物质可能渗入土壤和水源，造成长期的环境和健康危害。电子垃圾的焚烧会释放有毒气体，加剧空气污染，并使非正规回收作业中的工人暴露于有害物质。研究估计，全球只有 17.4% 的电子垃圾被正确收集和回收，大部分最终进入垃圾填埋场或缺乏适当环境保护的非正规废物处理场所[@singh2022disentangling]。

AI 硬件的复杂组成给回收带来了重大挑战。与拆卸相对简单的传统计算组件不同，AI 加速器采用了专门的多层电路、异种金属合金和紧密集成的内存架构，使得材料回收变得困难。拆卸和分离金、钯和稀土金属等有价值的元素需要尚未广泛普及的先进回收技术。混合材料的存在进一步使过程复杂化，因为某些组件以化学键合或嵌入的方式存在，使提取效率低下。

尽管面临这些挑战，各方正在努力开发 AI 硬件的可持续处置方案。一些制造商已经开始设计具有模块化架构的 AI 加速器，便于组件更换并延长设备的使用寿命。研究也在进行中，以改进材料回收工艺，使从废弃芯片中提取和再利用镓、铟和钨等重要元素成为可能。湿法冶金和生物冶金等新兴技术在以较低环境影响提取稀有金属方面显示出前景，优于传统的冶炼和精炼方法。

循环经济[^fn-circular-economy]模型为缓解与 AI 硬件相关的电子垃圾危机提供了一种有前景的方法。循环经济原则不遵循线性的"使用并丢弃"模式，而是强调再利用、翻新和回收，以延长计算设备的生命周期。Google 和 Microsoft 等公司已启动倡议，将退役的 AI 硬件重新用于次要应用，例如运行较低优先级的机器学习任务或将功能性组件重新分配给研究机构。这些努力有助于减少对新半导体生产的总体需求，同时最小化废物产生。

[^fn-circular-economy]: **循环经济（Circular Economy）**：通过资源的持续再利用来消除废物的经济模式，与线性的"获取-制造-丢弃"模式形成对比。由艾伦·麦克阿瑟基金会于 2010 年推广，现已纳入欧盟政策，目标是到 2035 年实现 65% 的回收率。对于电子产品而言，这意味着模块化、可维修性和材料回收的设计理念。戴尔的海洋塑料计划和 Fairphone 的模块化智能手机是循环经济原则的典范。在 AI 硬件领域，循环经济方法可以通过组件升级和二次使用案例将 GPU 寿命从 3-5 年延长至 8-10 年。

政策干预和监管框架对于解决 AI 系统的处置阶段至关重要，是对企业可持续发展倡议的补充。世界各国政府正在开始实施生产者责任延伸（EPR）政策，要求技术制造商对其产品在整个生命周期内的环境影响承担责任。在欧盟等地区，严格的电子废物管理法规要求电子产品制造商参与认证的回收计划，并确保有害物质的安全处置。然而，执法仍然不一致，全球电子废物的跟踪和管理存在重大缺口。

AI 硬件处置的未来将取决于回收技术的进步、执法力度以及行业对可持续设计原则的广泛采用。AI 驱动的电子垃圾日益紧迫的形势，凸显了整合生命周期管理策略的必要性，该策略需考虑从原材料提取到报废回收的 AI 基础设施全生命周期环境影响。如果不齐心协力提高硬件可持续性，AI 的快速扩张将继续对全球资源和废物管理系统施加压力。
...
### 监管机制与全球协调 {#sec-sustainable-ai-regulatory-mechanisms-global-coordination-4180}
...
#### 测量与问责 {#sec-sustainable-ai-measurement-accountability-d40b}

透明的测量和报告为可持续 AI 治理奠定了基础。如果没有标准化的跟踪机制，组织就无法准确评估环境影响或确定改进机会。

### 测量与报告 {#sec-sustainable-ai-measurement-reporting-798b}
...
### 限制机制 {#sec-sustainable-ai-restriction-mechanisms-a260}
...
### 政府激励措施 {#sec-sustainable-ai-government-incentives-face}
...
### 行业自律 {#sec-sustainable-ai-selfregulation-02df}
...
### 全球影响 {#sec-sustainable-ai-global-impact-aafd}
