...
class QMDTranslator:
    ...
    def _fix_quarto_newlines(self, content: str) -> str:
        ...
        # 3. ::: {attrs} 后面紧跟非空白内容 → 在后面插入空行
        #    如 '::: {callout}- item' → '::: {callout}\n\n- item'
        content = re.sub(r'(:::\s*\{[^}]*\})([^\n])', r'\1\n\2', content)
        ...
        # 5. 单独的 ::: 闭合标记前面不是空行 → 插入空行
        content = re.sub(r'([^\n])(\n):::(\s*\n)', r'\1\n\n:::\3', content)

        # 6. 单独的 ::: 闭合标记前面没有换行 → 插入空行
        content = re.sub(r'([^\n]):::(\s*\n)', r'\1\n\n:::\2', content)

        # 7. 单独的 ::: 闭合标记后面紧跟非空白内容 → 插入空行
        content = re.sub(r'(:::)(\s*\n)([^\n\s])', r'\1\n\n\3', content)
...
6. 特别注意：Quarto 指令标记（::: 开头的行）已被替换为 __QD_PH_N__ 占位符，占位符内部的 title 等属性也会被保留，不要在翻译时添加或修改 :::
7. 如果原文中两个占位符之间有换行（如 __QD_PH_1__\\n\\n__QD_PH_2__），译文中必须保持完全相同的换行结构
8. 当看到 __QD_PH_N__"英文文本"__QD_PH_M__ 的模式时（即两个 __QD_PH__ 占位符之间夹着引号包裹的文本），引号内的文本需要翻译，但引号和占位符必须原样保留
